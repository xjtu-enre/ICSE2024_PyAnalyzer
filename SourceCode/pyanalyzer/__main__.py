...
from pyanalyzer.analysis.analyze_manager import AnalyzeManager
from pyanalyzer.cfg.Resolver import Resolver
from pyanalyzer.cfg.module_tree import Scene
from pyanalyzer.passes.aggregate_control_flow_info import aggregate_cfg_info
from pyanalyzer.vis.representation import DepRepr
from pyanalyzer.vis.summary_repr import from_summaries, call_graph_representation
...
def main() -> None:
    ...
    start = time.time()
    manager = pyanalyzer_wrapper(root_path, config.compatible, config.cfg, config.cg, config.builtins)
    end = time.time()
...
def dump_call_graph(project_name: str, resolver: Resolver) -> None:
    ...
    out_path = f"{project_name}-call-graph-PyAnalyzer.json"
    with open(out_path, "w") as file:
        json.dump(call_graph, file, indent=4)


def pyanalyzer_wrapper(root_path: Path, compatible_format: bool, need_cfg: bool, need_call_graph: bool,
    ...
    manager.work_flow()
    out_path = Path(f"{project_name}-report-PyAnalyzer.json")
    if need_cfg:
